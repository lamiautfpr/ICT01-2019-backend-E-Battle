...
exports.handler = async (event) => {
    ...
    let results = null, results2 = null;
    let id = null;
        ...
        case 'POST':
            ...
            if (!(body.game && body.spaces && body.groups)){
                return {
                    ...
                    body: JSON.stringify({
                        errorCode: 1,
                        errorMessage: "Faltam dados na requisição, consulte a documentação"
                    })
                }
            }
            ...
            if(body.groups.length < 2){
                return {
                    ...
                    body: JSON.stringify({
                        errorCode: 1,
                        errorMessage: "Não é possivel realizar a requisição com apenas um grupo"
                    })
                }
            }
            ...
            try{
                results = await conn.query({
                    ...
                    values: [body.game, body.spaces, JSON.stringify(body.groups), body.random ?? false, body.trivia ?? false],
                });
            }catch(e){

                if( e.message == "insert or update on table \"matches\" violates foreign key constraint \"matches_games_fk\""){
                    return {
                        statusCode: 400,
                        body: JSON.stringify({
                            errorCode: 1,
